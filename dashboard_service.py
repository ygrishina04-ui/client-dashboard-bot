...
def build_from_user_files(uid: int):
    ...
    print(
        f"SNOOZE LOADED: {len(snoozed_clients)}",
        flush=True,
    )
...
async def rebuild_from_storage(bot):
    ...
    if missing:
        print(
            f"REBUILD: не хватает файлов {missing}",
            flush=True,
        )
        return False
    ...
    print(
        f"Дашборд восстановлен. OUTPUT={OUTPUT}",
        flush=True,
    )
...
# =========================================================
# TELEGRAM HANDLERS — DASHBOARD ONLY
# =========================================================

def register_dashboard_handlers(
    dp,
    bot,
    on_portfolio_uploaded=None,
):
    ...
    async def start(message: Message):
        ...
        await message.answer(
            ...
            "из файла <b>заказы</b>.\n\n"
            "Для контроля коммуникации доступны команды "
            "/register, /today и /status."
        )
    ...
    async def rebuild(message: Message):
        await message.answer(
            "Пробую восстановить последний дашборд..."
        )

        try:
            ok = await rebuild_from_storage(bot)
            ...
            await message.answer(
                f"Ошибка восстановления: <code>{e}</code>"
            )
    ...
    async def reset(message: Message):
        ...
        await message.answer(
            "Текущая загрузка сброшена. "
            "Пришлите заново 3 файла."
        )
    ...
    async def doc(message: Message):
        ...
        # ВАЖНО:
        # Портфель одновременно обновляет базу
        # "Внимание на клиента".
        if kind == "portfolio" and on_portfolio_uploaded:
            try:
                result = on_portfolio_uploaded(str(path))

                await message.answer(
                    "База для контроля коммуникации обновлена ✅\n"
                ...
                # Ошибка уведомлений НЕ ломает дашборд.
                await message.answer(
                    "⚠️ Портфель принят для дашборда, "
                    "но базу коммуникаций обновить не удалось:\n"
                    f"<code>{e}</code>"
                )
